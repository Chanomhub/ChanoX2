        ...
        <div className="flex h-full bg-background overflow-hidden">
            ...
            <div className="flex-1 min-w-0">
                {selectedGame ? (
                    ...
                    <div className="h-full overflow-y-auto p-6 custom-scrollbar bg-background">

                        {/* All Games Grid */}
                        <div>
                            <div className="flex items-baseline mb-4">
                                <h2 className="text-foreground font-bold text-sm mr-2">All Games</h2>
                                <span className="text-muted-foreground text-xs">({filteredGames.length})</span>
                            </div>
                            ...
                            {filteredGames.length === 0 ? (
                                ...
                                <div className="flex flex-wrap gap-4">
                                    {filteredGames.map(item => (
                                        ...
                                        >
                                            <div className="w-full h-[220px] bg-card rounded-lg border border-border/60 shadow-lg overflow-hidden relative mb-2">
                                                {item.coverImage ? (
                                                    <SafeImage
                                                        src={item.coverImage}
                                                ...
                                                {item.isReExtracting ? (
                                                    ...
                                                    <div className="absolute bottom-2 right-2 w-8 h-8 bg-primary rounded-full items-center justify-center hidden group-hover:flex shadow-md">
                                                        <Play className="w-3 h-3 text-primary-foreground fill-current ml-0.5" />
                                                    </div>
                                                )}
                                            </div>
                                            <span className="text-foreground text-[13px] font-medium truncate w-full text-left group-hover:text-primary transition-colors">
                                                {item.title}
                                            </span>
                                        </button>
                                    ))}
                                    ...
                                    <button className="w-[160px] h-[220px] border border-dashed border-border rounded-lg flex items-center justify-center text-muted-foreground text-[13px] hover:text-foreground hover:border-primary transition-colors">
                                        + Add shelf
                                    </button>
                                </div>
                            )}
